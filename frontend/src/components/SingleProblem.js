...
const SingleProblem = () => {
  ...
  const [editorTheme, setEditorTheme] = useState("vs-light"); // State for editor theme
  ...
  const runCode = () => {
    setLoading(true);
    axios
      .post(`${backendurl}/problem/runproblem`, { code: code[language], language, input })
      .then((res) => {
        if (res.data.output !== "") {
    ...
    <>
      ...
      {isloggedin && problem ? (
        <div className="w-screen h-screen overflow-hidden ">
          <div className="flex flex-col md:flex-row h-full mt-12">
            <div className="w-full md:w-1/2 h-full overflow-auto bg-white p-6">
              <div className="flex items-center justify-between mb-4 mt-6 mr-4">
